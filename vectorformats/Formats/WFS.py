...
class WFS(Format):
   ...
   xsi:schemaLocation="http://www.opengis.net/wfs http://schemas.opengeospatial.net//wfs/1.0.0/WFS-basic.xsd">
    ...
    def encode_transaction(self, response, **kwargs):
        ...
        if (len(insertResult) + len(updateResult) + len(replaceResult) + len(deleteResult)) == failedCount:
            result += "<wfs:FAILED/>"
        elif (len(insertResult) + len(updateResult) + len(replaceResult) + len(deleteResult)) > failedCount and failedCount > 0:
            result += "<wfs:PARTIAL/>"
        else:
            result += "<wfs:SUCCESS/>"
